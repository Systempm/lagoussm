    ...
    <td><%=list.get(i).getPositionName()  %></td>
         <td><%=list.get(i).getPositionAdvantage()  %></td>
             <td><%=list.get(i).getSalary()  %></td>
              ...
              <td><a href="Userinlove?username=<%=session.getAttribute("username") %>&positionid=<%=list.get(i).getPositionId() %>&page=<%=alldbPageVo.getPageNum() %>"> 收藏 </a> || <a href="Userinhate?username=<%=session.getAttribute("username") %>&positionid=<%=list.get(i).getPositionId()%>&page=<%=alldbPageVo.getPageNum() %>">厌恶</a> </td>   
  </tr>
  <% }%>
</table>
...
<%for(int j = alldbPageVo.getStart() ; j <alldbPageVo.getEnd()+1;j++){ %> 
 <a href="user_highselectdb?page=<%=j %>"target="fname"><%=j %></a>
<%} %>
...
<%for (int a = 1 ; a <alldbPageVo.getTotalPage();a++){ 
	...
	%><option value="user_highselectdb?page=<%=a %>" selected><%=a %>页</option>   
<%}else{%>

   <option value="user_highselectdb?page=<%=a %>"><%=a %>页</option>   
<%}} %>
</select>
</p>
